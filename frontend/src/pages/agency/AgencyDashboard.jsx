...
function AgencyDashboard() {
  const { properties, loading, error } = useListingStore();
  const { agency, agencyToken, fetchPropertiesByAgency } = useAgencyStore();
  ...
  useEffect(() => {
    if (agency?.id) {
      fetchPropertiesByAgency(agency.id, agencyToken);
    }
  }, [agency?.id, fetchPropertiesByAgency, agencyToken]);
    ...
    <>
      ...
      <div className="flex flex-col overflow-x-hidden">
        ...
        <div className="flex items-center justify-center pl-10 pr-10 font-raleway">
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 w-full gap-5 items-center">
            <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
              {agencyProperties.map((property) => (
                <PropertyTile
                  property={property}
                  key={property.id}
                  onEdit={() => navigate(`/agency/editproperty/${property.id}`)}
                />
              ))}
            </div>
          )}
